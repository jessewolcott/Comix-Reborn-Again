...
function Comix_OnLoad()
  ...
  ComixVersion = GetAddOnMetadata( 'Comix', 'Version' );
  ComixOptionsHeader = "Comix Options "..ComixVersion;

  -- Saying Hello --
  DEFAULT_CHAT_FRAME:AddMessage("Comix "..ComixVersion.." sais: Hello you there, this AddOn is enabled by default! Standby while I'm loading resources!",0.7,0.7,1);
  ...
  -- These might be deprecated after the unit speed check change, don't remove util code is cleaned up more --
  forward = false;
  back = false;
  left = false;
  right = false;
  ...
  -- Establish actual game version used for Classic / Retail API changes
  Comix_GameToc = select(4, GetBuildInfo());
  ...
  if Comix_GameToc < 20000 then
    SLASH_Comix5 = "/Comixclassic";
    SLASH_Comix6 = "/comixclassic";
  end
end
...
function PlayBounceSound()
  if Comix_AddOnEnabled then
    local boing, landed = false;

    -- Check if player is moving. Used for mount boings
    if GetUnitSpeed("player") > 0 then
      boing = true;
    end

    -- Check if player is already in the air, this is to prevent sounds while already jumping.
    ...
    if not UnitOnTaxi("player") then
      if IsMounted() then
        if Comix_GameToc >= 20000 then
          -- DEFAULT_CHAT_FRAME:AddMessage("After TBC - Flying exists",1,0,0);
          -- DEFAULT_CHAT_FRAME:AddMessage(Comix_GameToc,1,0,0);
          if not IsFlying() and landed and not IsSwimming() then
            -- DEFAULT_CHAT_FRAME:AddMessage("Triggers when jumping in TBC or later, not flying, not in the air, not swimming",1,0,0);
            if boing then
          ...
          if landed and not IsSwimming() then
            -- DEFAULT_CHAT_FRAME:AddMessage("Classic - Flying API doesn't exist yet",1,0,0);
            -- DEFAULT_CHAT_FRAME:AddMessage(Comix_GameToc,1,0,0);
            if boing then
              Comix_JumpCount = Comix_JumpCount + 1;
            end
        ...
        if Comix_GameToc < 20000 then
          if landed and not IsSwimming() then
            -- DEFAULT_CHAT_FRAME:AddMessage("Triggers when jumping unmounted in Classic, not in the air, not swimming",1,0,0);
            Comix_JumpCount = Comix_JumpCount + 1;
            if Comix_BounceSoundEnabled and landed then
              Comix_DongSound(ComixSpecialSounds, 8);
            end
          end
        else
          if not IsFlying() and landed and not IsSwimming() then
            -- DEFAULT_CHAT_FRAME:AddMessage("Triggers when jumping in TBC or later, unmounted, with extra check for druid flightform, not in the air, not swimming",1,0,0);
            Comix_JumpCount = Comix_JumpCount + 1;
...
-- These might be deprecated. Need to clean up more code to make sure --
...
function Comix_OnEvent(self, event, ...)
  ...
  if (Comix_AddOnEnabled) then
        ...
        if subEvent == "SWING_DAMAGE" then
          ...
          if critical == true then
            ...
            if Comix_CritGapEnabled == true then
              ...
              if damageAmount == nil or damageAmount < Comix_CritGap then
                return --nothin to see here
              end
    ...
    if event == "UNIT_HEALTH" then
      ...
      if Comix_FinishhimEnabled then
        if Comix_FinishTarget then
          if UnitHealth("target") > 0 and (not UnitIsFriend("player", "target")) then
    ...
    -- These ability triggers seem bugged. Need more testing, but doesn't look like the event throws any arg at all
    -- Might be misunderstanding the trigger, but doesn't seem right at least
    -- Ability Sounds Checks - well besides cannibalize and sprint/dash --
    if event == "UNIT_SPELLCAST_SENT" then
	  local spellID = select(4, ...)
	  local spellName = GetSpellInfo(spellID)
      --DEFAULT_CHAT_FRAME:AddMessage("Debug unit_spellcast_sent trigger - Arg2 = "..arg2,1,0,0)
      if Comix_DemoShoutEnabled then
        if spellName == COMIX_DS or spellName == COMIX_DR then
          Comix_Pic(0, 0,ComixSpecialImages[2])
        end
      end
      if Comix_AbilitySoundsEnabled then
        ...
        elseif spellName == COMIX_SPRINT or spellName == COMIX_DASH or spellName == COMIX_ASPECT or spellName == COMIX_STAMPEDING_ROAR then
    ...
    -- Hug Coounter / Objections .. FU String Manipulation!!! --
    if event == "CHAT_MSG_TEXT_EMOTE" then
      ...
      if arg1 ~= nil then
        ...
        if strfind(arg1, _G["COMIX_HUG"]) or strfind(arg1, _G["COMIX_HUGS"]) then
          ...
          local comix_the_hugger = nil
          local comix_the_hugged_one = nil
		  local guid = select(12, ...)
		  local playerName, playerRealm = select(6, GetPlayerInfoByGUID(guid))
		  local playerNameWithRealm = playerName.."-"..playerRealm
		  local comix_youup_trimmed = _G["COMIX_YOUUP"]:gsub("%s+", "")
		  local comix_youlow_trimmed = _G["COMIX_YOULOW"]:gsub("%s+", "")
          local huggerInTextStart, huggerInTextEnd = strfind(arg1, "[^ .,!?:;]+")
		  local gappie = nil

		  if huggerInTextStart then
		    -- get hugger
		    comix_the_hugger = strsub(arg1, huggerInTextStart, huggerInTextEnd)
			
			if strfind(comix_the_hugger, comix_youup_trimmed) then
              comix_the_hugger = UnitName("player")
            end

            -- get the hugged one
			if strfind(arg1, _G["COMIX_HUGS"]) then
              gappie = tonumber(_G["COMIX_HUGSGAP"])
            elseif strfind(arg1, _G["COMIX_HUG"]) then
              gappie = tonumber(_G["COMIX_HUGGAP"])
            end

		    if gappie then
		      gappie = gappie + huggerInTextEnd - huggerInTextStart + 1
              comix_the_hugged_one = strsub(arg1, strfind(arg1,"[^ .,!?:;]+",gappie))

			  if strfind(comix_the_hugged_one, comix_youlow_trimmed) then
                comix_the_hugged_one = UnitName("player")
              end
		    end
		  end

		  -- check if hugger / the hugged one already exists in list
          for i, line in ipairs(Comix_HugName) do
            if Comix_HugName[i] == comix_the_hugger then
          ...
          -- add hugger / the hugged one in list if not existing
          ...
          if HuggerNotFound then
            if not strfind(arg1, _G["COMIX_NEED"]) then
              Comix_HugName[getn(Comix_HugName)+1] = comix_the_hugger
          ...
          -- sort list by most hugged players
		  for i, line in ipairs(Comix_HugName) do
...
function Comix_Command(Nerd)
  ...
  elseif (Nerd == "about") then
    DEFAULT_CHAT_FRAME:AddMessage("[Comix]: Author: Darool @ Quel'Thalas & Nixxen / Version: "..ComixVersion,0,0.5,1)
...
function Comix_Fail(msg)
  if isWoWClassic then
	DEFAULT_CHAT_FRAME:AddMessage("[Comix]: /fail isn't available in classic era",0,0.5,1)
	return;
  end
  -- local link = GetSpellLink(65311) -- Even though GetSpellLink() is available in WotLKC, it doesn't return a valid link for spell id 65311.
  local name, _, icon, castTime, minRange, maxRange, spellID = GetSpellInfo(65311);
  local link = format("|c%s|Hspell:%d:0|h[%s]|h|r", "FF71D5FF", spellID, name);
  msgSender(msg,link)
end

function Comix_Awesome(msg)
  if isWoWClassic then
	DEFAULT_CHAT_FRAME:AddMessage("[Comix]: /awesome isn't available in classic era",0,0.5,1)
	return;
  end
  -- local link = GetSpellLink(58783) -- Even though GetSpellLink() is available in WotLKC, it doesn't return a valid link for spell id 58783.
  local name, rank, icon, castTime, minRange, maxRange, spellID = GetSpellInfo(58783);
  local link = format("|c%s|Hspell:%d:0|h[%s]|h|r", "FF71D5FF", spellID, name);
  msgSender(msg, link)
end
...
function msgSender(message,link)
  ...
  txt = gsub(txt,"~~",link)
  if command == "say" or command == "emote" or command == "yell" or command == "instance" or command == "party" or command == "guild" or command == "officer" or command == "raid" or command == "rw" or command == "bg" or command == "afk" or command == "dnd" then
    if command == "instance" then
      command = "instance_chat"
    end
    if command == "bg" then
...
function Comix_Pic(x,y,Pic, name)
  ...
  if Comix_ImagesEnabled then
    ...
    elseif name == "customcomixpic" then
...
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixImagesCt.." piccus",0.3,0.3,0.3)

-- Counting Images in Image-Sets --
ComixFireImagesCt = getn(ComixFireImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixFireImagesCt.." piccus",0.3,0.3,1)

ComixFrostImagesCt = getn(ComixFrostImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixFrostImagesCt.." piccus",0.3,1,1)

ComixFrostfireImagesCt = getn(ComixFrostfireImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixFrostfireImagesCt.." piccus",0.3,1,1)

ComixSpellstormImagesCt = getn(ComixSpellstormImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixSpellstormImagesCt.." piccus",0.3,1,1)
...
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixShadowImagesCt.." piccus",0.3,1,0.3)

ComixNatureImagesCt = getn(ComixNatureImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixNatureImagesCt.." piccus",1,1,0.3)

ComixArcaneImagesCt = getn(ComixArcaneImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixArcaneImagesCt.." piccus",1,0.3,0.3)

ComixHolyHealImagesCt = getn(ComixHolyHealImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixHolyHealImagesCt.." piccus",0.5,0.5,0.5)

ComixHolyDmgImagesCt = getn(ComixHolyDmgImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixHolyDmgImagesCt.." piccus",0.5,0.5,0.5)

ComixDeathImagesCt = getn(ComixDeathImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixDeathImagesCt.." piccus",0.5,0.5,0.5)

ComixOverkillImagesCt = getn(ComixOverkillImages)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixOverkillImagesCt.." piccus",0.5,0.5,0.5)

-- Counting Normal Sounds --
ComixSoundsCt = getn(ComixSounds)
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixSoundsCt.." sounds",1,0.3,1)
...
DEFAULT_CHAT_FRAME:AddMessage("Me loaded "..ComixSpecialCt.." specials",1,0.3,0.3)
...
DEFAULT_CHAT_FRAME:AddMessage("Open options GUI with /comix or get Slashcommands with /comix help",0.7,0.7,1)
